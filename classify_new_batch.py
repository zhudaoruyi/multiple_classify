...
# 导入模型
# model = load_model('/home/zhwpeng/abc/evaluate_model/models/finish_1228/m20032_inc_py27_01_p.h5')
# model = load_model('/home/zhwpeng/abc/evaluate_model/models/unfinish/m15032_inc_ex_01_p.h5')
# model = load_model('/home/zhwpeng/abc/evaluate_model/models/unfinish/m15032_010401_p.h5')
# model = load_model('/home/zhwpeng/abc/evaluate_model/models0112/m15064_in_01_l.h5', compile=False)
model = load_model('/home/zhwpeng/abc/evaluate_model/models/models0112/m15016_x_01_l.h5', compile=False)
model.compile(optimizer='sgd', loss='binary_crossentropy', metrics=['accuracy'])
# model = load_model('models/m20032_res_py27_01_p.h5')

# 需要分类的数据集
# test_dir = './data/EXTRACT_FROM_VERCTOR_PDF'
# test_dir = 'data/CHART_TYPE'
# test_dir = 'pickouts/results010401'
# test_dir = 'pickouts/results010501'
# test_dir = 'pickouts/results010502'
# test_dir = 'pickouts/results010802'
# test_dir = 'pickouts/results010902'
test_dir = 'pickouts/results011002'
# test_dir = '/home/zhwpeng/data/images0109/wechat'
# test_dir = '/home/zhwpeng/data/images0109/0111classi/wrong_classify/da0111'
...
def predict(test_set, batch_size=64, model=model):
    x = []
    for i, k in tqdm(enumerate(test_set)):
        img = cv2.imread(k)[:, :, ::-1]
...
def predict_on_batch(test_set, batch_size=64, model=model):
    predictions = []
    print 'start time is :', current_time()
    for start in tqdm(range(0, len(test_set), batch_size)):
        ...
        predictions.append(model.predict_on_batch(x_batch))
    print 'end time is :', current_time()
...
predictions = predict(test_set, batch_size=32)


if __name__ == '__main__':
    move_image_to_class(test_set, predictions, dst_dirs='results/lf_classify_da011002/')
...
#         pred_label = sorted(pred_label, reverse=True)
#
#         # dst_dirs = 'results/' + 'lf_classify_010501/'
#         # dst_dirs = 'results/' + 'CHART_TYPE_0105/'
#         # dst_dirs = 'results/' + 'lf_classify_010802/'
#         dst_dirs = 'results/' + 'lf_classify_da0111/'
#         # dst_dirs = '/home/zhwpeng/data/images0109/' + 'wechat_results/'
#         if not os.path.exists(dst_dirs):
#             os.mkdir(dst_dirs)
#
...
#             if not os.path.exists(dst_dirs + 'low_prob'):
#                 os.mkdir(dst_dirs + 'low_prob/')
#             shutil.copy(test_set[ids+start], dst_dirs + 'low_prob/')
#             low_prob_sum += 1
#         # 单类别的图片
#         if pred_label != [] and len(pred_label) == 1:
#             if not os.path.exists(dst_dirs + pred_label[0]):
#                 os.mkdir(dst_dirs + pred_label[0])
#             shutil.copy(test_set[ids+start], dst_dirs + pred_label[0])
#             single_sum += 1
#         # 双类别的图片
#         if pred_label != [] and len(pred_label) == 2:
#             if not os.path.exists(dst_dirs + str(pred_label[0] + '_and_' + pred_label[1])):
#                 os.mkdir(dst_dirs + str(pred_label[0] + '_and_' + pred_label[1]))
#             shutil.copy(test_set[ids+start], dst_dirs + str(pred_label[0] + '_and_' + pred_label[1]))
#             double_sum += 1
#         # 三类别的图片
#         if pred_label != [] and len(pred_label) == 3:
#             if not os.path.exists(dst_dirs + str(pred_label[0] + '_and_' + pred_label[1] + '_and_' + pred_label[2])):
#                 os.mkdir(dst_dirs + str(pred_label[0] + '_and_' + pred_label[1] + '_and_' + pred_label[2]))
#             shutil.copy(test_set[ids+start], str(pred_label[0] + '_and_' + pred_label[1] + '_and_' + pred_label[2]))
#             triple_sum += 1
#
# print '低概率图片数量是：%d' % low_prob_sum, '单类图片的数量是：%d' % single_sum, \
#     '双类图片的数量是：%d' % double_sum, '三类别图片的数量是：%d' % triple_sum
