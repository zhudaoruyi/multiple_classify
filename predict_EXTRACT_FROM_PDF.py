...
# 被测文件夹
test_dir = '/home/zhwpeng/data/EXTRACT_FROM_PDF/fromPDF/'
# 被测文件夹下的文件夹名
...
obj_dirs = ['area', 'bar', 'column', 'line', 'line+column', 'pie', 'radar', 'table', 'unknown', 'unknown_chart']
...
for ids, direcs in enumerate(obj_dirs):
    ...
    test_set = sorted(glob(test_dir + direcs + '/*'))
    ...
    if len(test_set):
        ...
        with open(dst_dir + obj_dirs[ids] + '.json', 'w') as fil:
